...
describe("dropdown.css", () => {
    ...
    describe(".dropdown", () => {
        ...
        it("has a solid 1px border using $form-border", () => {
            expect(css).toMatch(
                /\.dropdown\s*{[^}]*border:\s*1px\s+solid\s+\$form-border\s*;/,
            );
        });
        ...
        it("sets color from $looks-secondary", () => {
            expect(css).toMatch(
                /\.dropdown\s*{[^}]*color:\s*\$looks-secondary\s*;/,
            );
        });
    ...
    describe(".mod-open", () => {
        ...
        it("sets background-color using $form-border", () => {
            expect(css).toMatch(
                /\.mod-open\s*{[^}]*background-color:\s*\$form-border\s*;/,
            );
        });
    });
    ...
    describe(".dropdown-icon", () => {
        ...
        it("is square: width and height are both .5rem", () => {
            const width = css.match(
                /\.dropdown-icon\s*{[^}]*width:\s*([\d.]+rem)/,
            );
            const height = css.match(
                /\.dropdown-icon\s*{[^}]*height:\s*([\d.]+rem)/,
            );
        ...
        it("aligns vertically to middle", () => {
            expect(css).toMatch(
                /\.dropdown-icon\s*{[^}]*vertical-align:\s*middle\s*;/,
            );
        });

        it("has padding-bottom of .2rem", () => {
            expect(css).toMatch(
                /\.dropdown-icon\s*{[^}]*padding-bottom:\s*\.2rem\s*;/,
            );
        });
    });
    ...
    describe("Directional styles (LTR/RTL)", () => {
        it("defines LTR-specific rule for .dropdown-icon", () => {
            expect(css).toMatch(
                /\[dir="ltr"\]\s*\.dropdown-icon\s*{[^}]*margin-left:\s*\.5rem\s*;/,
            );
        });

        it("defines RTL-specific rule for .dropdown-icon", () => {
            expect(css).toMatch(
                /\[dir="rtl"\]\s*\.dropdown-icon\s*{[^}]*margin-right:\s*\.5rem\s*;/,
            );
        });

        it("uses the same spacing value for LTR and RTL margins", () => {
            const ltr = css.match(
                /\[dir="ltr"\]\s*\.dropdown-icon\s*{[^}]*margin-left:\s*([\d.]+rem)/,
            );
            const rtl = css.match(
                /\[dir="rtl"\]\s*\.dropdown-icon\s*{[^}]*margin-right:\s*([\d.]+rem)/,
            );
    ...
    describe(".mod-caret-up", () => {
        ...
        it("rotates caret by 180deg", () => {
            expect(css).toMatch(
                /\.mod-caret-up\s*{[^}]*transform:\s*rotate\(180deg\)\s*;/,
            );
        });

        it("sets padding-bottom to 0", () => {
            expect(css).toMatch(
                /\.mod-caret-up\s*{[^}]*padding-bottom:\s*0\s*;/,
            );
        });

        it("sets padding-top to .2rem and matches the base icon bottom padding", () => {
            const base = css.match(
                /\.dropdown-icon\s*{[^}]*padding-bottom:\s*([\d.]+rem)/,
            );
            const top = css.match(
                /\.mod-caret-up\s*{[^}]*padding-top:\s*([\d.]+rem)/,
            );
    ...
    describe("Consistency and conventions", () => {
        ...
        it("has no duplicate class rule declarations", () => {
            [
                ...
                const re = new RegExp(`^${cls.replace(".", "\\.")}\\s*{`, "gm");
